...
const StyledTab = styled(Tab)`
  ...
  z-index: ${(props) =>
    props.selected ? 2 : 1}; /* 선택된 탭이 다른 탭 위에 오도록 */
  background-color: ${(props) => (props.selected ? '#fff' : '#f1f1f1')};
  border-bottom: none;

  &:hover {
    background-color: #e0e0e0; /* hover 시 색상 변경 */
    cursor: url('https://unpkg.com/nes.css@latest/assets/cursor-click.png'),
      pointer; /* 커서를 NES 스타일로 변경 */
  }
...
const Mypage = () => {
  ...
  const groupedItems = userInfo
    ? userInfo.userItems.reduce((acc, item) => {
  ...
  const handleUpdate = async () => {
    // 필수 입력값 확인
    if (
      !updatedInfo.userRealname ||
      !updatedInfo.userNickname ||
      !updatedInfo.userBirthdate
    ) {
      alert('모든 필드를 입력해주세요.')
      return
    }
  ...
  const isValidBirthdate = (date) => {
    ...
    return (
      dateObj && dateObj.getMonth() + 1 === month && dateObj.getDate() === day
    )
  }
            ...
            {selectedTab === 0 && (
              <div className="mypage-content">
                <div className="profile-section">
                  ...
                  <div
                    className="profile-image-wrapper nes-pointer"
                    onClick={openProfilePictureModal}
                  >
                    <img
                      src={selectedPicture || userInfo.userPicture}
                      alt="프로필"
                  ...
                  <div className="profile-icon-wrapper">
                    <img
                      src={getTierIcon(userInfo.userGrade)}
                      alt="티어 아이콘"
                ...
                <div className="info-section">
                  ...
                  <div className="info-item">
                    ...
                    <span
                      className="info-value-item nes-pointer"
            ...
            {selectedTab === 1 && (
              <div className="modal-content">
                ...
                <div className="modal-item">
                  ...
                  {nicknameError && (
                    <div id="mypage-nick-error">{nicknameError}</div>
                  )}
                  {!nicknameError && isNicknameAvailable === false && (
                    <div id="mypage-nick-error">
                      이미 사용 중인 닉네임입니다...
                    </div>
                  )}
                  {!nicknameError && isNicknameAvailable === true && (
                    <div id="mypage-nick-success">
                      사용 가능한 닉네임입니다!!!
                    </div>
                  )}
                </div>
            ...
            {selectedTab === 2 && (
              <div>
                ...
                <div className="delete-content">
                  회원님이 등록하신 개인 정보는 삭제되어 재사용하지 않습니다.
                </div>
                <div className="delete-content">
                  회원 탈퇴 시 결제하신 상품과 재화는 복구되지 않습니다.
                </div>
                <div className="delete-content">
                  회원 탈퇴하시면 계정을 복구 받으실 수 없습니다.
                </div>
                <div className="delete-content">
                  주의 사항을 읽으시고 회원 탈퇴를 결심하셨다면 아래의 회원 탈퇴
                  버튼을 클릭해주세요.
                </div>
                <button
